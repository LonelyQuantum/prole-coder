...
| [ ] | P5-16c：自动推断 Chat run mode 并隐藏模式下拉框 | `vscode/extension/src/chatView.ts`、`vscode/extension/src/chatInput.ts`、`docs/vscode-extension.md` | 未完成：Sidebar composer 不再默认暴露 `edit` / `ask` / `plan` / `review` 下拉框；发送时根据用户文本和轻量显式前缀自动推断 run mode，问答走 `ask`，实现/修复走 `edit`，计划讨论走 `plan`，代码审查走 `review`。保留高级/调试入口用于强制指定 mode，并补充 TS 与 extension-host 回归测试，确保自然语言覆盖不会破坏现有 RPC `mode` 协议。 |
| [ ] | P5-16d：PowerShell 验证命令规则与假失败收敛 | `docs/turn-loop.md`、`docs/tool-system.md`、`crates/agent-core/src/turn_loop.rs` | 未完成：根据真实试用中 `cargo test 2>&1` 在 Windows PowerShell 5.1 下把 CLIXML/progress 噪声带入 stderr、导致“stdout 全绿但工具状态 failed”的现象，补强 provider 工具使用契约：模型不应为测试/验证命令手动追加 `2>&1`，应让 shell 工具分别捕获 stdout/stderr；必要时在 shell 工具或风险/提示层增加可测试保护，避免 PowerShell redirection 把成功验证误判为失败。验收：新增 prompt/context 或 shell 行为测试，覆盖 Windows PowerShell 下测试命令不推荐 `2>&1`，并确保 Output/run log 仍保留真实 stderr。 |
