...
- VS Code 插件中的 DeepSeek API key 存入 SecretStorage，优先级高于进程环境变量；传给 RPC 子进程时只通过 child env 覆盖 `DEEPSEEK_API_KEY`。DeepSeek model ID 不是密钥，可通过 VS Code 普通设置保存，并在 child env 中覆盖 `DEEPSEEK_MODEL`。
- `.env` 和本地状态必须被 git 忽略。
